"""
Game engines for Tic Tac Toe, Word Chain, Hangman
"""
import random
...
# ============ TIC TAC TOE ============
def create_tictactoe_state() -> Dict[str, Any]:
...
def make_tictactoe_move(state: Dict[str, Any], position: int, symbol: str) -> Tuple[bool, Optional[str]]:
    """
...
def format_tictactoe_board(board: list) -> str:
    """Format board for display"""
    symbols = [b if b else ' ' for b in board]
    return (
...
HARD_WORDS = [
    ...
    "flamingo", "kangaroo", "octopus", "universe", "volcano", "waterfall"
]


def create_wordchain_state(difficulty: str) -> Dict[str, Any]:
...
def make_wordchain_move(state: Dict[str, Any], word: str) -> Tuple[bool, Optional[str], str]:
    """
...
HANGMAN_WORDS = [
    ...
    "garden", "rating", "message", "profile", "settings", "keyboard",
    "button", "command", "handler", "database", "server", "network"
]


def create_hangman_state() -> Dict[str, Any]:
...
def make_hangman_guess(state: Dict[str, Any], letter: str) -> Tuple[bool, bool, Optional[str]]:
    """
    Make hangman guess
...
def format_hangman_word(state: Dict[str, Any]) -> str:
...
def format_hangman_display(state: Dict[str, Any]) -> str:
    """Format full hangman display"""
    word_display = format_hangman_word(state)
    guessed = ', '.join(sorted(state['guessed_letters'])) if state['guessed_letters'] else 'none'
    
    return (
        f"Word: {word_display}\n"
        f"Wrong guesses: {state['wrong_guesses']}/{state['max_wrong']}\n"
        f"Guessed: {guessed}"
    )
