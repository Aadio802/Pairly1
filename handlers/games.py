"""
Game handlers - Tic Tac Toe, Word Chain, Hangman
"""
...
from db.sunflowers import get_sunflower_balance, deduct_sunflowers_smart, add_sunflowers
from services.game_engine import (
    create_tictactoe_state, make_tictactoe_move, format_tictactoe_board,
    create_wordchain_state, make_wordchain_move,
    create_hangman_state, make_hangman_guess, format_hangman_display,
    get_next_player
)
...
async def game_accept_callback(callback: CallbackQuery):
    ...
    partner_id = await get_partner(accepter_id)
    ...
    if game_type == 'tictactoe':
        ...
        await start_wordchain(callback.bot, game_id, inviter_id, accepter_id, initial_state)
    elif game_type == 'hangman':
        await start_hangman(callback.bot, game_id, inviter_id, accepter_id, initial_state)
...
async def tictactoe_move_callback(callback: CallbackQuery):
    ...
    if winner:
        ...
        if winner == 'draw':
            ...
            await callback.bot.send_message(winner_id, f"🎉 You won! +{game['bet_amount'] * 2 + settings.GAME_BASE_REWARD} 🌻")
            await callback.bot.send_message(loser_id, f"😔 You lost. -{game['bet_amount']} 🌻")
        
        await callback.message.edit_text(
            f"Game Over!\n\n{format_tictactoe_board(game['state']['board'])}",
            reply_markup=None
        )
    else:
        # Continue
        await callback.message.edit_reply_markup(reply_markup=board_markup)
        turn_name = "Your" if next_turn == user_id else "Partner's"
        await callback.answer(f"{turn_name} turn")


# ============ WORD CHAIN ============
async def start_wordchain(bot, game_id: int, player1_id: int, player2_id: int, state: dict):
...
# Word chain moves handled in text message handler
@router.message(F.text)
async def wordchain_text_handler(message: Message):
    """Handle word chain text input"""
    user_id = message.from_user.id
    chat_id = await get_chat_id(user_id)
    ...
    if not game or not game['game_type'].startswith('wordchain'):
        return
    ...
    # Make move
    valid, error, last_letter = make_wordchain_move(game['state'], word)
    ...
    # Update state
    partner_id = await get_partner(user_id)
    await update_game_state(game['game_id'], game['state'], partner_id)
    
    # Notify
    await message.answer(f"✅ Good! Next letter: '{last_letter}'")
    await message.bot.send_message(
        partner_id,
        f"Partner played: {word}\nYour turn! Send word starting with '{last_letter}'"
    )


# ============ HANGMAN ============
async def start_hangman(bot, game_id: int, player1_id: int, player2_id: int, state: dict):
    """Start hangman game"""
    display = format_hangman_display(state)
    
    msg = f"🔤 Hangman started!\n\n{display}\n\nSend a letter to guess!"
    
    await bot.send_message(player1_id, msg)
    await bot.send_message(player2_id, msg)


# Hangman handled in text handler (similar to word chain)
